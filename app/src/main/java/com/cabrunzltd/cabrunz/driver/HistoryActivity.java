...
import com.cabrunzltd.cabrunz.driver.adapter.HistoryAdapter;
...
public class HistoryActivity extends ActionBarBaseActivitiy implements
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		((TextView) mDialog.findViewById(R.id.payment_type))
				.setVisibility(View.GONE);
		LinearLayout promolayout = (LinearLayout) mDialog
				.findViewById(R.id.promolayout);
		if (history.getPromo().equals("1")) {
			promolayout.setVisibility(View.VISIBLE);
			((TextView) mDialog.findViewById(R.id.tvactualPrice)).setText(" "
					+ history.getActual_price());
			((TextView) mDialog.findViewById(R.id.tvdiscountPrice)).setText(" "
					+ history.getPromo_discount());

		} else promolayout.setVisibility(View.GONE);
		...
		if (timeCost.equals("0.00")) {
			...
			((TextView) mDialog.findViewById(R.id.tvBillTimePerHour))
					.setText(String.valueOf(perHourFormat.format((Double
							.parseDouble(timeCost) / Double.parseDouble(history
							.getTime()))))
							+ " "
							+ getResources().getString(
									R.string.text_cost_per_min));
